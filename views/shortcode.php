    ...
    public function plugin()
    {
        ...
        $v = '<div class="f13-wordpress-container">';
            $v .= '<div class="f13-wordpress-header">';
                ...
                $v .= '<a class="f13-wordpress-slug" href="'.$this->plugins_url.esc_attr($this->data->slug).'" target="_blank" title="'.sprintf($this->label_on_wordpress, esc_attr($this->data->name)).'">';
                    $v .= esc_attr($this->data->name);
                $v .= '</a>';
            $v .= '</div>';
            $v .= '<div class="f13-wordpress-description">';
                ...
                $v .= '<div class="f13-wordpress-rating">';
                    ...
                    $v .= ' ('.sprintf($this->label_rating_from_ratings, round(esc_attr($this->data->rating) / 20, 1), esc_attr($this->data->num_ratings)).')';
                $v .= '</div>';
                $v .= '<div class="f13-wordpress-excerpt">';
                    $v .= '<strong>'.$this->label_description.':</strong>';
                    $v .= substr(wp_strip_all_tags($this->data->sections->description), 0, 200).'...';
                $v .= '</div>';
                $v .= '<div class="f13-wordpress-tags">';
                    $v .= '<strong>'.$this->label_tags.':</strong>';
                    $v .= '<div>';
                        foreach ($this->data->tags as $tag) {
                            $v .= '<span class="f13-wordpress-tag">'.esc_attr($tag).'</span>';
                        }
                    $v .= '</div>';
                $v .= '</div>';
                $v .= '<div class="f13-wordpress-download">';
                    $v .= '<strong>'.$this->label_downloads.':</strong>';
                    $v .= esc_attr($this->data->downloaded);
                $v .= '</div>';
                $v .= '<div class="f13-wordpress-hr"></div>';
                $v .= '<div class="f13-wordpress-links">';
                    $v .= '<a class="f13-wordpress-button" href="'.esc_url($this->data->download_link).'">';
                        $v .= sprintf($this->label_download_version, esc_attr($this->data->version));
                    $v .= '</a>';
                $v .= '</div>';
                $v .= '<details class="f13-wordpress-requirements">';
                    ...
                    $v .= '<div>';
                        $v .= '<span class="f13-wordpress-requirement">';
                            $v .= '<strong>'.$this->label_version.':</strong>'.esc_attr($this->data->version);
                        $v .= '</span>';
                        $v .= '<span class="f13-wordpress-requirement">';
                            $v .= '<strong>'.$this->label_requires_wordpress.':</strong>'.esc_attr($this->data->requires).'+';
                        $v .= '</span>';
                        $v .= '<span class="f13-wordpress-requirement">';
                            $v .= '<strong>'.$this->label_tested_with_wordpress.':</strong>'.esc_attr($this->data->tested);
                        $v .= '</span>';
                        $v .= '<span class="f13-wordpress-requirement">';
                            $v .= '<strong>'.$this->label_php_version.':</strong>'.esc_attr($this->data->requires_php).'+';
                        $v .= '</span>';
                        $v .= '<span class="f13-wordpress-requirement">';
                            $v .= '<strong>'.$this->label_created.':</strong>'.date('F j, Y', strtotime(esc_attr($this->data->added)));
                        $v .= '</span>';
                        $v .= '<span class="f13-wordpress-requirement">';
                            $v .= '<strong>'.$this->label_last_updated.':</strong>'.date('F j, Y g:ia', strtotime(esc_attr($this->data->last_updated)));
                        $v .= '</span>';
                    $v .= '</div>';
                $v .= '</details>';
            $v .= '</div>';
